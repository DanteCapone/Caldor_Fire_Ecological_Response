...
OUT_DIR <- Sys.getenv(
  "CALDOR_TAXA_PROFILE_OUT_DIR",
  unset = file.path("figures", "supplemental", "taxa_profiles")
)
...
clean_ltp <- raw %>%
  ...
  mutate(
    year      = year(date),
    month     = month(date),
    # 1 mm3 = 1e9 um3; abundance is cells L^-1 and unit_biovol is um3 cell^-1.
    biovolume = abundance * unit_biovol / 1e9   # mm3 L^-1
  ) %>%
...
# Monthly abundance profiles in the same point-and-line style as the existing
# single-variable profile grids. A fixed abundance scale is used across months
# so depth-profile magnitudes remain directly comparable.
make_abundance_profile_figure <- function(taxon_name, taxon_col, taxon_label, year1, tag) {
  months_tbl <- make_month_seq(year1) %>%
    mutate(month_label = factor(
      paste(month.abb[month], year),
      levels = paste(month.abb[month], year)
    ))
  source_dat <- clean_ltp %>%
    filter(
      taxon == taxon_name,
      (year == year1 & month >= 4) | (year == year1 + 1L & month <= 4)
    )
  dat <- source_dat %>%
    group_by(year, month, depth_m) %>%
    summarise(
    ...
    inner_join(months_tbl, by = c("year", "month")) %>%
    arrange(month_label, depth_m)

  write_csv(
    dat %>% mutate(
      source_file = PHYTO_FILE,
      abundance_units = "cells L^-1"
    ),
    file.path(OUT_DIR, paste0("taxa_profiles_", tag,
                             "_abundance_data.csv"))
  )

  abundance_max <- max(dat$abundance, na.rm = TRUE)
  if (!is.finite(abundance_max) || abundance_max <= 0) abundance_max <- 1

  p <- ggplot(dat, aes(x = abundance, y = depth_m, group = month_label)) +
    geom_path(colour = taxon_col, linewidth = LO_LW_THICK, na.rm = TRUE) +
    geom_point(colour = taxon_col, fill = taxon_col, shape = 21,
               size = 2.2, stroke = 0.35, na.rm = TRUE) +
    facet_wrap(~month_label, ncol = 4, drop = FALSE) +
    scale_y_reverse(
      name = "Depth (m)", breaks = c(0, 20, 40, 60, 75, 90, 105),
      limits = c(105, 0),
      expand = expansion(mult = c(0.02, 0.02))
    ) +
    scale_x_continuous(
      name = expression("Abundance (cells L"^-1*")"),
      limits = c(0, abundance_max), breaks = scales::breaks_pretty(n = 3),
      labels = function(x) {
        labels <- scales::label_scientific(digits = 1)(x)
        labels[is.finite(x) & abs(x) < sqrt(.Machine$double.eps)] <- "0"
        labels
      },
    ...
    theme_bw(base_size = LO_FONT_MIN_PT, base_family = BASE_FAMILY) +
    theme(
      panel.grid.minor = element_blank(),
      strip.background = element_rect(fill = "grey94", colour = "grey75"),
      strip.text = element_text(face = "bold", size = LO_FONT_MIN_PT),
      axis.title = element_text(size = LO_FONT_MIN_PT),
      axis.text = element_text(size = LO_FONT_MIN_PT),
      axis.title.x = element_text(colour = taxon_col),
      axis.text.x = element_text(colour = taxon_col, angle = 35, hjust = 1),
  ...
  stem <- file.path(
    OUT_DIR,
    paste0("taxa_profiles_", tag, "_abundance_horizontal")
  )
  ragg::agg_png(paste0(stem, ".png"), width = LO_WIDTH_MAX,
                height = LO_HEIGHT_MAX, units = "cm", res = LO_DPI_LINE)
  print(p)
  invisible(dev.off())
  ggsave(paste0(stem, ".pdf"), p, width = LO_WIDTH_MAX,
         height = LO_HEIGHT_MAX, units = "cm", device = cairo_pdf)
  message("Saved L&O monthly abundance profiles: ", basename(stem), ".png/.pdf")
}
...
make_heatmap_figure <- function(taxon_name, taxon_col, taxon_label,
                                year1, variable, fname, fill_high = taxon_col) {
  months_tbl <- make_month_seq(year1) %>%
  ...
  fill_max <- max(grid_df$val, na.rm = TRUE)
  fill_breaks <- unique(c(0, fill_max))
  ...
  p <- ggplot(grid_df, aes(x = month_label, y = depth_m, fill = val)) +
    ...
    scale_fill_gradient(
      low = "white", high = fill_high, na.value = "grey92",
      name = vinfo$label, breaks = fill_breaks,
...
dual_profiles_only <- identical(Sys.getenv("CALDOR_DUAL_PROFILES_ONLY"), "1")
for (cmb in combos) {
  heatmap_plots <- list()
  if (!dual_profiles_only) {
    for (variable in c("abundance", "biovolume")) {
      ...
      heatmap_plots[[variable]] <- make_heatmap_figure(
        cmb$taxon, cmb$col, cmb$label, cmb$year1, variable,
        fname = paste0("taxa_profiles_", cmb$tag, "_", variable, "_heatmap.png"),
        fill_high = if (variable == "abundance") COL_LEPTO else "#0072B2"
      )
    }
  }

  make_abundance_profile_figure(cmb$taxon, cmb$col, cmb$label, cmb$year1, cmb$tag)
}
